...
export async function GET(req: NextRequest) {
  ...
  if (type === 'all' || type === 'barcodes') {
    let q = sb
      .from('dna_barcodes')
      .select(
        'id, species_name, scientific_name, family, habitat_category, coast, marker_type, sequence_length, genbank_accession, collection_location, collection_date, source, status',
      )
