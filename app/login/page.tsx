...
function logStorage(label: string) {
  ...
  console.log(`[login] ${label} localStorage keys:`, keys);
  keys.forEach((k) => {
    const v = localStorage.getItem(k);
    console.log(`[login] ${label} ${k}:`, v ? `${v.slice(0, 50)}...` : null);
  });
}
...
export default function LoginPage() {
  ...
  async function signInWithGoogle() {
    ...
    setError(null);
    try {
      logStorage('before signInWithOAuth');
      ...
      logStorage('after signInWithOAuth');
      if (error) {
      ...
      console.log('[login] signInWithOAuth result:', { url: data?.url });
      if (data?.url) {
      ...
      console.error('[login] signInWithOAuth exception:', err);
