-- ============================================================
-- COMPLETE FIX: Marine fish genomics tables + seed data
...
CREATE INDEX IF NOT EXISTS idx_genome_assemblies_species ON public.genome_assemblies(species_slug);
CREATE INDEX IF NOT EXISTS idx_genome_assemblies_habitat ON public.genome_assemblies(habitat_category);
CREATE INDEX IF NOT EXISTS idx_genome_assemblies_status  ON public.genome_assemblies(status);

ALTER TABLE public.genome_assemblies ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS "Public read published assemblies" ON public.genome_assemblies;
CREATE POLICY "Public read published assemblies"
  ON public.genome_assemblies FOR SELECT
  USING (status = 'published');
DROP POLICY IF EXISTS "Authenticated insert" ON public.genome_assemblies;
...
CREATE INDEX IF NOT EXISTS idx_dna_barcodes_species ON public.dna_barcodes(species_slug);
CREATE INDEX IF NOT EXISTS idx_dna_barcodes_marker  ON public.dna_barcodes(marker_type);
CREATE INDEX IF NOT EXISTS idx_dna_barcodes_status  ON public.dna_barcodes(status);

ALTER TABLE public.dna_barcodes ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS "Public read approved barcodes" ON public.dna_barcodes;
CREATE POLICY "Public read approved barcodes"
  ON public.dna_barcodes FOR SELECT
  USING (status IN ('approved', 'published'));
DROP POLICY IF EXISTS "Authenticated insert barcodes" ON public.dna_barcodes;
CREATE POLICY "Authenticated insert barcodes"
  ON public.dna_barcodes FOR INSERT
  WITH CHECK (auth.role() = 'authenticated');

-- ── genomics_submissions ─────────────────────────────────────
...
CREATE INDEX IF NOT EXISTS idx_genomics_submissions_status ON public.genomics_submissions(status);
CREATE INDEX IF NOT EXISTS idx_genomics_submissions_email  ON public.genomics_submissions(submitter_email);

ALTER TABLE public.genomics_submissions ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS "Users read own submissions" ON public.genomics_submissions;
